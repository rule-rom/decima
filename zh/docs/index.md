...
- **PHASE_READ:** 数据 ("水") 填充公共 VSB 总线。所有 tiles 同时访问这个流，但只有具有 BUS_R 标志或 locked 祖先的 tiles 可以读取。如果祖先失去 FUSE (锁存状态) — 通过 decay 或域中的竞争者 — 后代也失去访问。输入流通过权重注入累加器，填充或将其驱动到负值。
- **PHASE_WRITE:** 具有在个性架构定义的阈值内的累加器值的 tiles 锁存，输出模式或在后续 ticks 中为它们的后代打开总线访问。
...
| **确定性重置** | 非活动 tile 强制归零 (thr_cur16=0, locked=0) |
