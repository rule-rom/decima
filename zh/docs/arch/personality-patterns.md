...
```mermaid
flowchart LR
    A["满量程过大"] --> B["作用停留在 Level 0..2"]
    B --> C["器官失聪"]
    D["满量程匹配"] --> E["使用 Level 0..15 的有效范围"]
    E --> F["器官能够区分事件强度"]
    G["满量程过小"] --> H["作用停留在 Level 14..15"]
    H --> I["器官饱和并产生噪声"]
```

迟滞可以消除阈值附近重复的 crossing 事件，但不能修复错误的尺度。失聪输入经过施密特
触发器后仍然失聪；饱和输入仍会产生物理上无法区分的事件。
