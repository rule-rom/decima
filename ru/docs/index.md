...
- **PHASE_READ:** Данные («вода») заполняют общую шину VSB. Все тайлы одновременно получают доступ к этому потоку, но читать могут лишь те, у которых есть флаг BUS_R или защелкнутый предок. Если предок теряет FUSE (защелкнутое состояние) — по decay или от конкурентов в домене — потомки также теряют доступ. Входной поток через веса проливается в аккумулятор, наполняя или уводя его в минус.
- **PHASE_WRITE:** Тайлы, имеющие значение аккумулятора в заданном архитектурой личности пороге, защелкиваются, выдавая паттерн или открывая доступ к шине на следующих тактах своим потомкам.
...
| **Выделенная линия Pattern ID** | Отдельный канал uint16 для выдачи паттернов, не забивает основную шину |
...
| **Наследование доступа** | Если предок расфьюзился — потомки начинают копить аккумуляторы с нуля; если зафьюзился снова — получают доступ |
