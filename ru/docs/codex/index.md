# Decima-8: Нейроморфная архитектура, оперирующая не битами, а уровнями энергии
...
3. Двухфазный цикл READ → WRITE с фиксированным временем 20 мкс. Никаких стохастических задержек. Никаких «в зависимости от нагрузки».
...
### 2.3 Функция активации: Clamp15 + нормализация

После вычисления `row_raw_signed[r]` мы должны преобразовать его в выходной сигнал для шины VSB. Здесь происходит магия детерминизма.

**На шину VSB (только неотрицательный):**
...
| --- | ---------- | ----- |
| max(..., 0) | Отсекает отрицательные суммы | Если торможение победило → выход 0 |
| + 7 | Сдвиг для округления| (x + 7) / 8 = округление вверх |
| / 8 | Нормализация диапазона | [-840..+840] → [0..15] |
| clamp15 | Жёсткое ограничение 0..15 | Защита от переполнения |
...
→ max(500, 0) = 500
→ (500 + 7) / 8 = 63.375
→ clamp15(63.375) = 15  ← насыщение

row_raw_signed[r] = +50
→ max(50, 0) = 50
→ (50 + 7) / 8 = 7.125
→ clamp15(7.125) = 7  ← нормальное значение

row_raw_signed[r] = -100
→ max(-100, 0) = 0
→ (0 + 7) / 8 = 0.875
→ clamp15(0.875) = 0  ← полное подавление
```

**В аккумулятор (signed, без clamp):**

```
// row_raw_signed[r] ∈ [-840..+840] используется как есть
// Сумма по всем 8 строкам: delta_raw ∈ [-6720..+6720]
```

> *💭 Ключевой момент: На шину VSB идут только неотрицательные значения (0..15), но в аккумулятор идут signed значения (-840..+840). Это позволяет реализовывать торможение без отрицательных сигналов на шине. Шина «не знает» о торможении — она видит только уровни энергии. Торможение живёт внутри тайла, в аккумуляторе.*
...
Если нужен другой динамический диапазон — настройте `weights`, `thr_lo/hi`, `decay16`.
Архитектура даёт рычаги, а не «магическую адаптивность».

**Вывод:** Мы выбираем предсказуемость и простоту. Гибкость достигается не «умной архитектурой», а конфигурацией параметров тайла.
...
graph TD
    ...
    subgraph TaktN ["ТАКТ N: Фиксация детекторов (FUSE)"]
        direction TB
        N1["🎹 Входной аккорд"] ==> N2["🔍 Тайлы-детекторы (BUS_R=1)"]
        N2 --> N3{"Соответствие?"}
        N3 -- Да --> N4["🔒 LOCK (FUSE)<br/>Открытие пути антагонисту"]
        N3 -- Нет --> N5["⚪ Пассивное состояние"]
    end

    subgraph TaktN1 ["ТАКТ N+1: Дифференциальный отбор"]
        direction TB
        M1["🔁 Повтор того же аккорда"] --> M2["🛡️ Тайлы-антагонисты"]
        M2 --> M3{"Условия LOCK?"}
        M3 -- "Паттерн наш" --> M4["🎯 LOCK + ВЫДАЧА ID"]
        M3 -- "Паттерн конкурента" --> M5["🚫 Блокировка выдачи"]
    end

    %% Межтактовая связь
    N4 ==>|"Активация антагониста"| M2

    %% Стилизация
    style TaktN fill:#f0f7ff,stroke:#0366d6,stroke-width:2px
    style TaktN1 fill:#fff7e6,stroke:#ffa940,stroke-width:2px
    
    style N4 fill:#e6ffed,stroke:#52c41a,font-weight:bold
    style M4 fill:#f6ffed,stroke:#52c41a,stroke-width:3px,font-weight:bold
    style M5 fill:#fff1f0,stroke:#f5222d
    style M3 fill:#fffbe6,stroke:#ffe58f
```
...
- Дирижёр подаёт один и тот же входной аккорд два такта подряд
- В первый такт решения не выдаются (только вычисления, накопление)
- Во второй такт — финальная проверка и выдача результата
...
- Если антагонист активен → блокировка детектора
...
| Конфигурация | Время цикла | FPS | Throughput |
|-------------|------------|-----|------------|
| **256 тайлов** (8×32) | **~22 мкс** | 15,783 | 0.063 Mb/s |
| **512 тайлов** (16×32) | **~43 мкс** | 11,111 | 0.043 Mb/s |
| **1024 тайла** (16×64) | **~81 мкс** | 6,200 | 0.023 Mb/s |
| **2048 тайлов** (32×64) | **~160 мкс** | 3,150 | 0.012 Mb/s |
| **4096 тайлов** (32×128) | **~311 мкс** | 1,620 | 0.006 Mb/s |
...
Это **предсказуемое поведение**. Никаких сюрпризов, никаких «внезапных просадок» при определённом размере.
...
> 💭 **Сравнение:** На GPU (CUDA/OpenCL) вы получите «среднее время 50 мкс ± 30 мкс». У нас: **81 мкс, точка**. Детерминизм — это наша суперсила.

### Детерминизм ≠ Линейность

**Время цикла зависит от количества тайлов, но не зависит от их активности.**

| Конфигурация | Тайлов | Время цикла | Частота |
| ------------ | ------ | ----------- | ------- |
| 8×32 | 256 | 20 мкс | 50 kHz |
| 16×32 | 512 | ~40 мкс | 25 kHz |
| 16×64 | 1024 | ~80 мкс | 12.5 kHz | 
| 32×64 | 2048 | ~180 мкс | 5.5 kHz | 
| 32×128 | 4096 | 311 мкс | 3.2 kHz |

Наблюдение: После 1024 тайлов рост времени ускоряется (кэш-промахи, давление на память). Это физическое ограничение, а не алгоритмическое.

Важно: Для каждой конфигурации время константно независимо от активности. 100% загрузка сети не увеличивает задержку.
...
Файл `.d8p` (Decima 8 Package) — это контейнер для «личности» сварма. Внутри нет кода. Только данные.
...
flowchart TD
    subgraph Lifecycle ["🚀 ЖИЗНЕННЫЙ ЦИКЛ ЛИЧНОСТИ (.d8p)"]
        ...
        Step2["<b>2. ПОДПИСЬ PKI-КЛЮЧОМ</b><br/>━━━━━━━━━━━━━━━━━━━━━━━━━━━━<br/>• 🔑 Ключ по подписке Tile/Council<br/>• 🖋️ Криптографическая подпись<br/>• ✅ Гарантия авторства и целостности"]
...
**Статус:** *OPEN SOURCE* (MIT/Apache 2.0)
...
### 7.2 Доступ к Store: статус, а не донат

Покупка тарифа — это **не «донат за спасибо»**. Это получение **PKI-ключа** для подписи личностей и доступа к публикации в Store.

**Правило Store:**

> 🔐 **Без валидной PKI-подписи новая личность не принимается в Store.**
> Но если личность уже загружена — она **не удаляется**, даже если подписка истекла.

Это не paywall. Это **цепочка доверия**: вы подписываете код → сообщество доверяет автору.

**Тарифы (Boosty):**

| Тариф | Цена | PKI-ключ | Лимит загрузок | Комиссия Store | Статус |
|-------|------|----------|----------------|----------------|--------|
| 📡 **Observer** | Бесплатно | ❌ | 0 | — | Изучение |
| 🌾 **Seed** | 16 ₽/мес | ❌ | 0 | — | Поддержка |
| 🌱 **Gardener** | 256 ₽/мес | ❌ | 0 | — | Комьюнити |
| 🔗 **Tile** | 1,024 ₽/мес | ✅ Intermediate | 10 / мес | 30% | Разработчик |
| ⚡ **Cluster** | 4,096 ₽/мес | ✅ Enhanced | 50 / мес | 20% | Профи / Студия |
| 👑 **Council** | 16,384 ₽/мес | ✅ Council | ∞ | 10% | Лидер / Компания |

**Как это работает:**

1. **Получение ключа**: При подписке на Tile/Cluster/Council вы автоматически получаете PKI-ключ, привязанный к аккаунту.
2. **Подпись**: В IDE или CLI вы подписываете `.d8p` файл этим ключом (`d8p sign --key <your.key>`).
3. **Публикация**: Store проверяет подпись (цепочка до Root CA) и статус подписки. Если всё ОК — публикация.
4. **Стабильность**: Если подписка истекла, ваши **уже опубликованные** личности остаются в Store. Но загрузить новые или обновить старые нельзя до возобновления.

| Компонент | Проверка подписи | Зачем |
|-----------|-----------------|-------|
| **Эмулятор (open source)** | ❌ Не проверяет | Локальное использование, тестирование, форки |
| **Store (curated)** | ✅ Проверяет | Гарантия авторства и целостности для пользователей |
| **IDE (closed, free)** | ✅ Проверяет при публикации | Единый стандарт UX для авторов |

> 💭 **Аналогия:** Git-репозиторий может кто угодно форкнуть. Но «Verified» бейдж на GitHub — только после проверки.

### 7.3 Дорожная карта

**[СЮДА: TIMELINE диаграмма]**
...
| **Ткань** | 8×32 .. 32×128 | 64×64 / кластеры | Сложные иерархические паттерны |
...
- Скачайте эмулятор: [github.com/intent-garden/decima8](https://github.com/intent-garden/decima8)
...
| 🔗 **Эмулятор (GitHub)** | [github.com/intent-garden/decima8](https://github.com/intent-garden/decima8) |
| 🔗 **Bakery (мета-обучение)** | [bakery.rulerom.com](https://bakery.rulerom.com) |
| 🔗 **PKI-центр** | [pki.rulerom.com](https://pki.rulerom.com) |
| 🔗 **Boosty (поддержка)** | [boosty.to/rulerom](https://boosty.to/intent-garden) |
| 🔗 **libwui (UI-движок)** | [Хабр, 2023](https://habr.com) |
...
## 🌐 rulerom.com — это не только Decima-8

Мы строим экосистему инструментов для тех, кто ценит **контроль, эстетику и математику**.

| Проект | Что это | Для кого |
|--------|---------|----------|
| **Decima-8** | Нейроморфная архитектура с детерминированным циклом | Исследователи, робототехника, HFT |
| **garden-core** | Валидация кода через clangAST + EDN-интенты для ИИ | Разработчики, которым нужен детерминизм в коде |
| **audrey** | Ламповый усилитель с VU-метром и контролем смещения | Аудиофилы, кто слышит разницу между «цифрой» и «теплом» |
| **libWUI** | Легковесный UI движок на чистом C++ |

> 🎁 **Easter egg в audrey**: в нём есть «ламповая Decima» для автострейфа в шутерах. Потому что мы не можем иначе.

> 💭 **Общее у всех проектов**: детерминизм, прозрачность, уважение к пользователю. Никакого «магического ИИ», никаких чёрных ящиков.
...
A: Это режим, при котором один и тот же вход подаётся два такта подряд для повышения селективности. 
   Используется при распознавании похожих паттернов с малым расстоянием Хэмминга (символы, цифры).
   В IDE включается галочкой «Double Strait», в .d8p выставляется флаг BAKE_FLAG_DOUBLE_STRAIT.
   Цена: 2 такта вместо 1. Выгода: удвоенная активация детектора + блокировка антагонистом.
