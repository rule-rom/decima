...
Все основные схемы можно сначала прожить руками в
[интерактивной WebGL-лаборатории](../pattern-lab.md), а затем сверить с
формальными правилами ниже.
...
их физического диапазона с VSB. Уровень `15` обязан означать определённую
физическую силу воздействия, а не случайный максимум конкретного файла.
...
Базовая нормализация и семантика линий остаются замороженными внутри
проверяемого контура. Но динамический диапазон живого источника может меняться
сильнее, чем рабочая область абсолютных порогов тайлов. В этом случае перед
swarm применяется причинный, ограниченный и полностью воспроизводимый
[AGC](agc.md).

AGC не меняет назначение линий и не выбирает правильное событие. Он удерживает
вход возле рабочей точки личности. Его алгоритм, параметры, состояние и warmup
становятся частью runtime manifest; без них `.d8p` уже недостаточно для
воспроизведения контура.

```mermaid
flowchart LR
    A["Слишком большой full scale"] --> B["Воздействие остаётся на Level 0..2"]
    B --> C["Орган глух"]
    D["Согласованный full scale"] --> E["Используется рабочая часть Level 0..15"]
    E --> F["Орган различает силу события"]
    G["Слишком малый full scale"] --> H["Воздействие постоянно на Level 14..15"]
    H --> I["Орган насыщен и шумит"]
```

Гистерезис устраняет повторные crossing-события возле порога, но не исправляет
неверный масштаб. Глухой вход после триггера Шмитта останется глухим, а
насыщенный будет выдавать редкие, но всё ещё физически неразличимые события.

## Сброс как управление восприимчивостью

Reset не является технической уборкой после FIRE. Он вмешивается в физику
следующего решения: удаляет часть накопленного состояния, снимает защёлки и
тем самым меняет чувствительность личности к будущему входу.

Два byte-identical органа после общей предыстории могут ответить на один и тот
же тестовый импульс по-разному, если один из них был сброшен:

```text
response = F(current_stimulus, baked_personality, accumulated_state,
             gain, decay, reset_history)
```

Это даёт архитектору несколько разных механизмов:

- **refractory:** победивший орган временно теряет накопленную готовность и не
  создаёт серию одинаковых FIRE;
- **rearm:** новый цикл становится возможен только после физически заданного
  возврата состояния;
- **смена гипотезы:** FIRE одного домена очищает память конкурирующей версии
  происходящего;
- **локальная новая история:** сбрасывается не вся личность, а только домены,
  для которых началось новое причинное окно.

Слишком частый reset делает орган глухим: поток не успевает накопиться.
Слишком редкий оставляет старую историю в новом событии и способен удерживать
орган возле порога. Поэтому политика сброса проектируется вместе с весами,
утечкой и коридором, записывается в runtime manifest и проверяется replay.

В сценарии [Reset и восприимчивость](../pattern-lab.md)
два одинаковых органа получают одну финальную посылку. Отличается только их
прожитое состояние, поэтому различается и результат.
...
8. Нормализация и состояние AGC записаны в manifest и воспроизводятся без
   будущих данных.
